...
public class FamilyTableModel extends AbstractTableModel {
	...
	/**
	 * Columns should be: -Family Number (Family table Primary key, ID) -Last
	 * Name (Family table) -Primary Name (Family table) -Number of Adults
	 * (Family table) -Number of Children (Family table) -Current Phone Number
	 * (Family_Phone_Number table) -Current Address (Family_Address)
	 */
	String[] columnNames = { "Select", "ID", "Last Name", "Primary",
			"# Adults", "# Children", "Phone", "Address" };

	Object[][] data;

	/**
	 * Generated Serial Version UID ... Probably unnecessary for this
	 * application
	 */
	...
	public FamilyTableModel() {
		 ...
		 */
		String sql = "SELECT f.Family_Number, f.Last_Name, f.Primary_Name, f.No_Children, f.No_Adults, ifnull(fp.Phone_Number,0),"
		...
		try {
			...
			while (result.next()) {
				...
				this.data[row][7] = result.getInt(7) + " "
