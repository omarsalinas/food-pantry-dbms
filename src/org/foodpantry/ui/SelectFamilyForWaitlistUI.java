...
/**
 * Window that lets the user choose someone to add to the waitlist. TODO needs
 * to enforce that family credentials are good before adding
 */
public class SelectFamilyForWaitlistUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs and adds all needed elements. Also, packs the frame. Does not,
	 * however, set the frame to visible.
	 * 
	...
	public SelectFamilyForWaitlistUI() throws HeadlessException {
		...
		scrollPane.setBorder(BorderFactory.createTitledBorder(
				scrollPane.getBorder(), "Families", TitledBorder.LEFT,
				TitledBorder.TOP));
		...
		/*
		 * pane layout +------------ | buttonPane |------------ | scrollPane
		 * +------------
		 */
		int padding = 5;
		// Edit family constraints
		layout.putConstraint(SpringLayout.WEST, buttonPane, padding,
				SpringLayout.WEST, pane);
		layout.putConstraint(SpringLayout.NORTH, buttonPane, padding,
	...
	/**
	 * Create a pane that displays the following: -"Add Family to List" button
	...
	private void addFamiliesToWaitlist() {
		/*
		 * The SQL statement should read something like: INSERT INTO
		 * Visit_Pantry VALUES (today, fam#, time, #inLine), VALUES (today,
		 * fam#, time, #inLine);
		 */
		...
		// Iterate over the list of families and put the selection(s) on the
		// wait list
		...
		try {
			...
			System.out
					.println("SelectFamilyForWaitlistUI: Failed to execute query");
